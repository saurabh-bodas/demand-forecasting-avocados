...
6. An **ensemble** of SUR and ARIMA models, which is a simple average across the two
...
# Example of a time series: Albany region

```{r}
#plotting each individual time series
for(reg in unique(avocado$region)[30:32]){
...
```{r echo=T, results='hide', warnings='hide'}
...
SUR_clean = data.frame(exp(SUR_pre_transform),week =seq.Date(from = as.Date("2017-08-06"),by = 7, 
                                      length.out = 34))
...
```{r }
#ensemble
compare_final$ensemble <- rowMeans(subset(compare_final, select = c(SUR, arima)), na.rm = TRUE)

#rmse ensemble
ensemble_rmse = compare_final %>% group_by(type,region)%>%summarise(ensemble = mean((ensemble - actual)^2)^0.5) %>% unite(region_type, region, type, sep='_')
...
#mape ensemble
ensemble_mape = compare_final %>% group_by(type,region)%>%summarise(ensemble = mean(abs(ensemble - actual)/actual)*100) %>% unite(region_type, region, type, sep='_')
...
compare_long = gather(compare_final, key='model',value = 'predicted', - c(region,type,week)) 
```
...
```{r fig.width=8, fig.height=5}
x=compare_long%>%filter(region=='Boise',type=='organic',model %in% c('SUR','ensemble','arima','actual'))
